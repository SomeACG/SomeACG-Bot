...
export async function hasPermisson(user_id:number, permisson: AdminPermission): Promise<boolean> {
    let document = await AdminModel.findOne({
        user_id: user_id
    })
    if(!document) return false
    if(document.permissions.includes(AdminPermission.DEFAULT) && permisson != AdminPermission.GRANT) return true
    return document.permissions.includes(permisson)
}

export async function hasPermissons(user_id:number, permissons: Array<AdminPermission>): Promise<boolean> {
    let document = await AdminModel.findOne({
        user_id: user_id
    })
    if(!document) return false
    let flag = true
    for(let permission of permissons)
    {
...
export async function grantPermissons(admin: AdminUser): Promise<boolean> {
    ...
    if(document)
    {
        let modified = await AdminModel.updateOne({
            user_id: admin.user_id
        }, admin)
...
export async function removePermissions(user_id: number): Promise<boolean> {
    let delete_result = await AdminModel.deleteOne({
        user_id: user_id
    })
