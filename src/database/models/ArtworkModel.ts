...
const artworkSchema = new Mongoose.Schema<Artwork>({
    ...
    tags: [new Mongoose.Schema<ArtworkTag>({
        _id: String,
        name: String
    })],
...
artworkSchema.pre('save', function (next) {
    let document: Mongoose.Document = this
    ConfigModel.findOneAndUpdate({}, { $inc: { artwork_count: 1 } }, {}, function (err, counter) {
        if (err) return next(err)
        document.$set({ index: counter.artwork_count + 1 })
        next()
    })
})
