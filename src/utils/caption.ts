...
function escapeHtmlTags(text: string) {
    return replaceHtmlBrackets(replaceHtmlBr(text));
}
/**
...
export function artworkCaption(
    ...
    if (artwork.desc)
        caption += `\n<blockquote expandable>${escapeHtmlTags(
            artwork.desc
        )}</blockquote>\n`;
...
export function infoCmdCaption(artwork_info: ArtworkInfo) {
    ...
    if (artwork_info.desc)
        caption += `<blockquote expandable>${escapeHtmlTags(
            artwork_info.desc
        )}</blockquote>\n`;
