...
export default wrapCommand('grant', async ctx => {
    if (!ctx.reply_to_message?.from) return await ctx.directlyReply('命令语法不正确，请回复一条其他用户的消息!')
    if (!ctx.command.target) return await ctx.directlyReply('命令语法不正确，请指定要授予的权限!')
    let user_id = ctx.reply_to_message.from.id
    let str_array = ctx.command.target.search(',') == -1 ? [ctx.command.target] : ctx.command.target.split(',')
    ...
    if (succeed) return await ctx.directlyReply(`成功将 ${permissons.toString()} 权限授予用户 <a href="tg://user?id=${user_id}">${ctx.reply_to_message.from.id}</a>`, 'HTML')
