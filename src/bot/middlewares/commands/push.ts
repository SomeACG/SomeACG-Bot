...
export default wrapCommand('push', async ctx => {
    if (!ctx.command.target || !ctx.command.params)
        return await ctx.directlyReply('命令语法不正确\n命令语法: /push [args(key=value)] <target>')
    if (!ctx.command.params['tags']) return await ctx.directlyReply('请至少设置一个标签!')
    if (ctx.reply_to_message && (ctx.reply_to_message as Message.DocumentMessage).document == undefined)
    ...
    let result = await publishArtwork(artwork_info, {
        ...
        picture_index: ctx.command.params['picture_index'] ? parseInt(ctx.command.params['picture_index']) : 0,
        artwork_tags: tags_string.search(',') == -1 ? [tags_string] : tags_string.split(/,|，/),
        origin_file_id: ctx.reply_to_message ? (ctx.reply_to_message as Message.DocumentMessage).document.file_id : undefined,
