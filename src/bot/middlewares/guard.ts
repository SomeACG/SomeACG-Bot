...
export function genAdminPredicate(permisson?: AdminPermission): AsyncPredicate<Context> {
    return async function (ctx: Context): Promise<boolean> {
        ...
        let has_permisson = await hasPermisson(ctx.from.id, permisson)
...
let adminPredicate: AsyncPredicate<Context> = async function (ctx: Context): Promise<boolean> {
    if (ctx.from && config.ADMIN_LIST.includes(ctx.from?.id.toString())) return true
...
let NonAdminHandler = Telegraf.fork(async ctx => {
    if (ctx.callbackQuery) {
        ctx.answerCbQuery("此操作仅限管理员使用")
    }
    if (ctx.message) {
